...
public:
	...
	void SetupAt(Bitu _phys_page,PageHandler * _old_pagehandler) {
		...
		SDL_memset(&hash_map,0,sizeof(hash_map));
		SDL_memset(&write_map,0,sizeof(write_map));
	...
	void writeb(PhysPt addr,Bitu val){
		...
		if (!host_readb(&write_map[addr])) {
			...
			SDL_memset(invalidation_map,0,4096);
		}
...
static void cache_init(bool enable) {
	...
	if (enable) {
		...
		if (cache_blocks == NULL) {
			...
			SDL_memset(cache_blocks,0,sizeof(CacheBlockDynRec)*CACHE_BLOCKS);
...
//#ifndef IPHONEOS
// #if (C_HAVE_MPROTECT)
			if(mprotect((char *)cache_code_link_blocks,CACHE_TOTAL+CACHE_MAXSIZE+PAGESIZE_TEMP,PROT_WRITE|PROT_READ|PROT_EXEC))

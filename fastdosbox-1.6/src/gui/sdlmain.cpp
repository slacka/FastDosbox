...
void GFX_SetTitle(Bit32s cycles,Bits frameskip,bool paused){
	...
	if(frameskip != -1) internal_frameskip = frameskip;
		sprintf(title,"FastDOSBox %s",VERSION);
...
static void GUI_StartUp(Section * sec) {
   ...
   if(sdl.desktop.want_type==SCREEN_OPENGL){ /* OPENGL is requested */
	sdl.surface=SDL_SetVideoMode(640,400,0,SDL_OPENGL); // 640x400
	if (sdl.surface == NULL) {
	...
	sdl.surface=SDL_SetVideoMode(640,400,0,0); // 640x400
...
/* Please leave the Splash screen stuff in working order in DOSBox. We spend a lot of time making DOSBox. */
	SDL_Surface* splash_surf = SDL_CreateRGBSurface(SDL_SWSURFACE, 640, 400, 32, rmask, gmask, bmask, 0); // 640x400
	if (splash_surf) {
		SDL_FillRect(splash_surf, NULL, SDL_MapRGB(splash_surf->format, 0, 0, 0));

		Bit8u* tmpbufp = new Bit8u[640*400*3];
		GIMP_IMAGE_RUN_LENGTH_DECODE(tmpbufp,gimp_image.rle_pixel_data,640*400,3); //640x400
...
static void show_warning(char const * const message) {
	...
	printf(message);
	if(textonly) return;
	if(!sdl.surface) sdl.surface = SDL_SetVideoMode(640,400,0,0); // 640x400
...
//	__asm __volatile ("swi 0x58ec1 ");
	...
	LOG_MSG("FastDOSBox version %s",VERSION);
	LOG_MSG("Ported by Norisc-NoFun, Based of DOSBox source code.");
